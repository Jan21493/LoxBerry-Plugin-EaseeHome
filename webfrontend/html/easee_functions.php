...
function easee_invalidate_token($url_base, $accessToken, $file_token)
{
    ...
    if (is_string($accessToken) && $accessToken !== '') {
        ...
        $accountId = null;
        if (is_array($profile) && isset($profile['userId']) && is_numeric($profile['userId'])) {
            $candidateAccountId = intval($profile['userId']);
            if ($candidateAccountId > 0) {
...
// Observation ids that the status page is able to display.
function easee_get_status_observation_ids()
{
    return array(
